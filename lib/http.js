...
export async function requireAdmin(req) {
  ...
  /* admins/{email} doc id-এর কেস আলাদা হতে পারে (Firebase email সব lowercase, কেউ
     Console থেকে `Admin@X.com` বানালে exact match fail → পুরো panel "Missing or
     insufficient permissions"/"admin নন") → id ধাপে ধাপে + `email` field query fallback। */
  let d = null;
  if (a.email) {
    const col = getDb().collection('admins');
    const lower = String(a.email).toLowerCase();
    const ids = [...new Set([a.email, lower].filter(Boolean))];
    for (const id of ids) {
      const sn = await col.doc(id).get();
      if (sn.exists) { d = sn.data() || {}; break; }
    }
    if (!d) {
      /* doc id পুরোপুরি আলাদা (যেমন uid দিয়ে বানানো) হলে collection টা ছোট —
         একবার স্ক্যান করে email (lowercase) মিলিয়ে নেওয়া হয় */
      try {
        const all = await col.limit(200).get();
        const hit = (all.docs || []).find(x => {
          const dd = x.data() || {};
          return String(dd.email || x.id || '').toLowerCase() === lower;
        });
        if (hit) d = hit.data() || {};
      } catch (_) { /* scan fail করলে পরের ধাপে 403 — ভুল admin কেও ঢুকতে দেওয়া হয় না */ }
    }
  }
  /* Admin Management → Suspend করলে doc থাকলেও access বন্ধ (সব admin op একই gate ধরে) */
  return { ...a, isAdmin: !!d && d.isAdmin !== false && !d.suspended, suspended: !!(d && d.suspended), adminData: d || null };
}
