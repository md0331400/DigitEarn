...
export default async function handler(req, res) {
  ...
  if (!list && !single && what !== 'jobs' && what !== 'wallet' && what !== 'admins' && what !== 'admin-joins') return fail(res, 400, 'Unknown read target');
  ...
  try {
    ...
    /* Admin Management: Owner + Full Access সব admin দেখে (suspend/balance সহ),
       Job Poster শুধু নিজেকে দেখে। role সবসময় server doc থেকেই। */
    if (what === 'admins' || what === 'admin-joins') {
      const { walletView } = await import('./wallet.js');
      const { roleOf } = await import('../../src/core/microjobs.js');
      const { canManage } = await import('./team.js');
      const meData = ((await db.collection('admins').doc(String(admin.email)).get())?.data?.()) || {};
      const isManager = canManage(roleOf(meData));
      const isOwner = roleOf(meData) === 'owner';

      /* pending join applications — শুধু manager */
      if (what === 'admin-joins') {
        if (!isManager) return fail(res, 403, 'শুধু Owner / Full Access admin — আবেদন দেখতে পারেন');
        const jSnap = await db.collection('adminJoins').limit(200).get();
        const whenMs = v => {
          if (!v) return 0;
          if (typeof v.toDate === 'function') { const x = v.toDate(); return x instanceof Date ? x.getTime() : 0; }
          return Number.isFinite(Number(v)) ? Number(v) : 0;
        };
        const items = (jSnap.docs || []).map(d => {
          const v = d.data() || {};
          return {
            email: d.id, fullName: String(v.fullName || '').slice(0, 60),
            requestedRole: v.requestedRole === 'full' ? 'full' : 'poster',
            note: String(v.note || '').slice(0, 300), status: String(v.status || 'pending'),
            attempts: Number(v.attempts) || 1, createdAt: v.createdAt || null, updatedAt: v.updatedAt || null,
            reviewedBy: v.reviewedBy || null, reviewNote: String(v.reviewNote || ''),
            alreadyAdmin: false, _t: Math.max(whenMs(v.createdAt), whenMs(v.updatedAt)),
          };
        });
        const adminSnap = await db.collection('admins').limit(400).get();
        const emails = new Set((adminSnap.docs || []).map(x => String(x.id).toLowerCase()));
        for (const it of items) it.alreadyAdmin = emails.has(String(it.email).toLowerCase());
        items.sort((a, b) => (a.status === 'pending' ? 0 : 1) - (b.status === 'pending' ? 0 : 1) || b._t - a._t);
        for (const it of items) delete it._t;
        return ok(res, { ok: true, what: 'admin-joins', isManager, isOwner, count: items.length, items });
      }
      ...
      if (!isManager) items = items.filter(x => String(x.email).toLowerCase() === String(admin.email).toLowerCase());
      items.sort((a, b) => (b.isOwner ? 1 : 0) - (a.isOwner ? 1 : 0) || String(a.email).localeCompare(String(b.email)));
      return ok(res, { ok: true, what: 'admins', isOwner, isManager, count: items.length, items });
    }
