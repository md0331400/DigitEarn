...
import { fail, ok, requireAdmin, authenticate, authReject, AUTH_OK, cors } from '../http.js';
  ...
  try {
    ...
    for (let i = 0; i < userDocs.length; i += BATCH) {
      ...
      const results = await Promise.all(batch.map(async ({ uid, data: u }) => {
        try {
          const n = await db.collection('users').doc(uid).collection('targetNotices').limit(20).get();
