...
import {
  ...
  ROLE_OWNER, ROLE_POSTER, ROLE_FULL, effectiveRole, roleOf, requiresBalance, posterRewardOk,
...
import { canManage, provisionAdmin, rejectJoin, setSuspended, emailOk } from './team.js';
...
function cleanTaskFields(body) {
  ...
  for (const k of TASK_KEYS) {
    ...
    else if (k === 'dailyLimit') out.dailyLimit = num(v, 0, 200, 0);   /* 0 = আনলিমিটেড */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (what === 'admin-balance' || what === 'admin-role' || what === 'admin-mode') {
      ...
      /* balance: Owner + Full Access ("full control") — Job Poster নিজে/কারও balance ছুঁতে পারবে না।
         role/mode: শুধু Owner (নাহলে Full Access নিজেদেরকে Owner বানিয়ে ফেলত) */
      if (what === 'admin-balance' ? !canManage(myRole) : myRole !== ROLE_OWNER) {
        return fail(res, 403, what === 'admin-balance'
          ? 'শুধু Owner / Full Access admin — Job Poster-এর balance যোগ বা কাট করতে পারেন'
          : 'শুধু Owner/Main Admin — role ও mode বদলাতে পারেন');
      }
      if (what === 'admin-mode') {
    ...
    /* ---- Admin Management: Join admin আবেদন approve/reject, সরাসরি admin তৈরি,
       suspend/unsuspend — Owner + Full Access only (Job Poster → 403) ---- */
    if (what === 'admin-create' || what === 'join-approve' || what === 'join-reject' || what === 'admin-suspend') {
      const meDoc = await adminDoc(db, admin.email);
      if (!canManage(roleOf(meDoc))) {
        return fail(res, 403, 'শুধু Owner / Full Access admin — admin account, approve ও suspend করতে পারেন');
      }
      const target = str(body.email, 120).toLowerCase();
      if (!emailOk(target)) return fail(res, 400, 'Admin email ঠিক করে লিখুন');

      if (what === 'join-reject') {
        const reason = str(body.reason || body.note, 300);
        if (reason.length < 3) return fail(res, 400, 'বাতিল করার কারণ লিখুন — আবেদনকারী এই কারণই দেখবে');
        const r = await rejectJoin({ db, email: target, reason, actorEmail: admin.email });
        return ok(res, { ok: true, ...r });
      }
      if (what === 'admin-suspend') {
        const r = await setSuspended({
          db, email: target, suspended: body.suspended !== false, reason: str(body.reason, 300),
          actorEmail: admin.email, myRole: roleOf(meDoc),
        });
        return ok(res, { ok: true, ...r });
      }

      const wantOwner = str(body.role, 12).toLowerCase() === 'owner';
      if (wantOwner && roleOf(meDoc) !== ROLE_OWNER) return fail(res, 403, 'Owner role শুধু Owner দিতে পারেন');

      if (what === 'join-approve') {
        const j = ((await db.collection('adminJoins').doc(target).get())?.data?.()) || null;
        if (!j) return fail(res, 404, 'আবেদনটা পাওয়া যায়নি (সম্ভবত আগেই handle করা হয়েছে)');
        if (String(j.status) !== 'pending') {
          return fail(res, 409, 'এই আবেদনটা আগেই ' + (j.status === 'approved' ? 'অনুমোদিত' : 'বাতিল') + ' হয়েছে');
        }
        const rr = str(body.role, 12).toLowerCase();
        const role = wantOwner ? 'owner' : (rr === 'full' ? 'full' : (rr === 'poster' || j.requestedRole === ROLE_POSTER) ? 'poster' : 'full');
        const r = await provisionAdmin({
          db, email: target, fullName: str(body.fullName || j.fullName, 60), role,
          /* grant = নতুন admin-এর শুরুর ব্যালেন্স (money credit, verdict না) */
          balance: num(body.grant, 0, 10_000_000, 0), actorEmail: admin.email, fromJoin: target,
        });
        return ok(res, { ok: true, approved: true, ...r });
      }

      /* direct create (Owner/Full Access নিজের হাতে) — email আগে থেকে admin হলে 409 */
      const already = await db.collection('admins').doc(target).get();
      if (already.exists) return fail(res, 409, 'এই email দিয়ে admin account আগেই আছে');
      const rr2 = str(body.role, 12).toLowerCase();
      const role2 = wantOwner ? 'owner' : (rr2 === 'full' ? 'full' : 'poster');
      const pw = str(body.password, 64);
      if (pw && pw.length < 8) return fail(res, 400, 'Password কমপক্ষে ৮ অক্ষর (খালি রাখলে setup link দেওয়া হবে)');
      const r = await provisionAdmin({
        db, email: target, fullName: str(body.fullName, 60), role: role2,
        balance: num(body.grant, 0, 10_000_000, 0), password: pw, actorEmail: admin.email,
      });
      return ok(res, { ok: true, created: true, ...r });
    }

    /* ---- user-এর private notice/warning — panel থেকেও server দিয়েই লেখা হয় ---- */
    if (what === 'target-notice-add' || what === 'target-notice-update' || what === 'target-notice-delete') {
      const uid = str(body.uid, 128);
      if (!/^[A-Za-z0-9_-]{2,128}$/.test(uid)) return fail(res, 400, 'Invalid uid');
      const nref = db.collection('users').doc(uid).collection('targetNotices');
      if (what === 'target-notice-add') {
        const n = (body.notice && typeof body.notice === 'object') ? body.notice : {};
        const title = str(n.title, 120);
        const text = str(n.body, 600);
        if (!title) return fail(res, 400, 'শিরোনাম লিখুন');
        if (!text) return fail(res, 400, 'নোটিশ/সতর্কবার্তা লিখুন');
        const id = 'tn_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
        const doc0 = {
          id, title, body: text,
          type: n.type === 'warning' ? 'warning' : 'notice',
          targetType: 'user', targetUserId: uid, enabled: true, sort: 10,
          createdBy: String(admin.email || '').toLowerCase(),
          createdAt: FieldValue.serverTimestamp(),
        };
        if (n.expiresAt) doc0.expiresAt = str(n.expiresAt, 40);
        await nref.doc(id).set(doc0);
        return ok(res, { ok: true, id, uid });
      }
      const nid = str(body.id, 80);
      if (!ID_RE.test(nid)) return fail(res, 400, 'Invalid notice id');
      if (what === 'target-notice-update') {
        await nref.doc(nid).update({ enabled: body.enabled === true, updatedAt: FieldValue.serverTimestamp() });
        return ok(res, { ok: true, id: nid });
      }
      await nref.doc(nid).delete();
      return ok(res, { ok: true, id: nid, deleted: true });
    }
