  ...
  try {
    ...
    if (what === 'task-create') {
      ...
      const doc = {
        ...
        requiredUsers: requiredUsers({ requiredUsers: data.requiredUsers }),
        /* MicroJob mode: এক user একবারই — approved হলে ওই user থেকে job লুকানো */
        mode: out.mode || MODE_SINGLE,
        approvedCount: 0, pendingCount: 0, closed: false,
        createdAt: now, updatedAt: now, createdBy: admin.uid,
      };
      await ref.set(doc);
      return ok(res, { ok: true, slug, created: true, mode: doc.mode, requiredUsers: doc.requiredUsers });
    }
    ...
    if (what === 'leaderboard-backfill') {
      ...
      for (const d of snap.docs || []) {
        const uid = d.id;
        try {
          const team = await db.collection('users').doc(uid).collection('team').limit(1000).get();
          await db.collection('users').doc(uid).set({ refCount: (team.docs || []).length }, { merge: true });
          updated++;
        } catch (_) { failed++; }
      }
      return ok(res, { ok: true, updated, failed, scanned: (snap.docs || []).length });
    }
